...
class World:
    ...
    def step(self,
        ...
        if self.st:
            ...
            self.m_tes_in, self.m_tes_out = self.st.step(self.timestep, self.T_amb, self.m_tes_in, self.m_tes_out, self.T_prd_wh)
        ...
        if self.battery:
            violation = self.battery.step(self.timestep, self.p_bat_in, self.p_bat_out)
            if violation:
        ...
        if self.num_prd > 0:
            ...
            _, self.turbine_power_output_MWe, self.turbine_power_generation_MWh, _, self.T_inj = \
                self.pp.power_plant_outputs(self.timestep, self.m_market, self.m_wh_to_turbine, self.m_tes_out, self.T_prd_wh, self.T_tes_out, self.T_amb)

            # Absolute geothermal capacity potential for capacity revenue calculation
            _, self.effective_ppc, _, _, _ = \
                self.pp.power_plant_outputs(self.timestep, self.m_g, self.m_g, 0.0, self.T_prd_wh, self.T_tes_out, self.T_amb)

            # Step reservoir
            self.reservoir.step(self.timestep, self.m_prd, self.m_inj, self.T_inj, self.T_amb)
        ...
        # return self.state, self.net_power_output_MWe, self.net_power_generation_MWh, self.T_inj
    ...
    def compute_economics(self):
        ...
        Cpumpsprod = np.sum(1.5*(1750*(prodpumphp)**0.7 + 5750*(prodpumphp)**0.2  + 10000 + self.reservoir.pumpdepth*50*3.281)) #see page 46 in user's manual assuming rental of rig for 1 day.
    ...
    def _reset(self):
        ...
        if self.reservoir_type == "energy_decline":
            ...
            self.reservoir = PercentageReservoir(Tres_init=self.Tres_init, geothermal_gradient=self.geothermal_gradient, surface_temp=self.surface_temp, 
                            ...
                            prd_well_diam=self.prd_well_diam, inj_well_diam=self.inj_well_diam, num_prd=self.num_prd, 
                            num_inj=self.num_inj, waterloss=self.waterloss,
                            power_plant_type=self.power_plant_type, pumpeff=self.pumpeff, PI=self.PI, II=self.II, SSR=self.SSR, drawdp=self.drawdp, plateau_length=self.plateau_length)
        ...
        self.df_market = self.df_market[(self.df_market.year >= self.start_year) & (self.df_market.year < self.end_year)].copy()
        self.df_market["capacity_price"] = self.df_market["year"].apply(lambda t: self.market.get_capacity_price(t))
        self.df_market["battery_elcc"] = self.df_market["year"].apply(
            lambda t: self.market.get_elcc(t, self.battery_duration[int(t-self.start_year > self.battery_lifetime)]) if self.battery else 0.0)
        self.df_weather = pd.DataFrame(np.tile(self.weather.df.values, (self.L, 1)), columns = self.weather.df.columns)
        self.df_market["T0"] = self.df_weather["T0"].values
        self.step_idx = 0
        self.state = self.df_market.iloc[self.step_idx]
        self.max_simulation_steps = len(self.df_market) - 1
